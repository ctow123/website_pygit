...
const Comments = ({ classes, ...props }) => {
  ...
  useEffect(() => {
    // console.log(user);
    socket.current = new WebSocket('ws://localhost:8000/apidb/ws/chat')
...
const commentToHTML = (commentarray) => {
    ...
    for (let i = 0; i < commentarray.length; i++) {
      ...
      temparray.push( < Typography key={i} color = "textPrimary"
        variant = "body1" > { commentarray[i].username + ' : ' + commentarray[i].comment } <
        /Typography>);
      }
...
// react cant render objects so if using {commentfromDB} can't store json in there
  return (
    <div className={`welcome tab`} style={{ backgroundColor: "white" }}>
      ...
      <Card className={classes.centered} style={{'max-height':'40vh', 'overflow':'auto'}} >
        <CardContent>
          {commentfromDB}
        </CardContent>
      </Card>
